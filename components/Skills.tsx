...
const skillGroups = [
  {
    ...
    color: "#4fffb0",
    skills: [
      { name: "React Native", level: 92, years: "4 tahun" },
      { name: "Expo", level: 85, years: "3 tahun" },
      { name: "React Navigation", level: 88, years: "4 tahun" },
      { name: "Redux / Zustand", level: 82, years: "3 tahun" },
    ],
  },
  {
...
const tools = [
  ...
  "Docker (basic)", "Jira", "Redux Toolkit", "Axios",
  "Firebase", "Vercel", "Railway", "OpenAPI/Swagger",
];

function SkillBar({ name, level, years, color }: { name: string; level: number; years: string; color: string }) {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "2rem", marginBottom: "3rem" }}>
          {skillGroups.map((group) => (
            <div key={group.category} style={{
              ...
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = `${group.color}33`)}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--border)")}
            >
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "1.5rem" }}>
                <div style={{
                  width: "8px", height: "8px", borderRadius: "50%",
                  background: group.color, boxShadow: `0 0 8px ${group.color}`,
                }} />
                <h3 style={{
